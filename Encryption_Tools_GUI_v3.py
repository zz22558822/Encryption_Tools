...
# 修改 log_action 函式，新增 key_identifier 參數
...
def encrypt_file_logic(file_path: str, password: str | None = None):
    ...
    加密後的檔案將包含原始檔案內容的 SHA256 雜湊值作為標頭。
    """
    ...
    # 計算原始檔案內容的雜湊值作為唯一識別碼
    original_file_content_hash = calculate_file_hash(data)
    ...
    try:
        with open(encrypted_path, 'wb') as f:
            # 新的加密檔案格式: [32 bytes 原始檔案雜湊][16 bytes 鹽值 (若有)][12 bytes nonce][加密數據]
            f.write(original_file_content_hash + salt + nonce + encrypted_data) 
...
def decrypt_file_logic(file_path: str, key_path_input: str = "", password: str | None = None):
    ...
    # 首先從加密檔案中提取原始檔案內容的雜湊值
    if len(full_content) < HASH_LENGTH:
        return {"status": "error", "message": "invalid_encrypted_file_format", 
                        "details": "加密檔案過短，無法提取內容雜湊值。"}
    
    extracted_hash = full_content[:HASH_LENGTH]
    ...
    # 如果有提供密碼，則嘗試用密碼解密
    if password:
        # 密碼加密模式下，內容順序: [32 bytes 原始檔案雜湊][16 bytes 鹽值][12 bytes nonce][加密數據]
        header_len = HASH_LENGTH + 16 + 12 # 雜湊 + 鹽 + nonce
        if len(full_content) < header_len:
            return {"status": "error", "message": "invalid_encrypted_file_format", 
                            "details": "檔案格式不符密碼加密模式，可能是隨機金鑰加密檔或已損壞。"}
        
        salt = full_content[HASH_LENGTH : HASH_LENGTH + 16]
        nonce = full_content[HASH_LENGTH + 16 : HASH_LENGTH + 16 + 12]
        encrypted_data = full_content[header_len:]
        ...
        try:
            ...
            key_identifier_for_log = "password" # 密碼解密的金鑰識別碼
        except Exception as e:
            return {"status": "error", "message": "decryption_failed", 
                            "details": f"密碼不正確或檔案已損壞: {str(e)}"}
    else: # 沒有提供密碼，嘗試用金鑰檔解密
        # 隨機金鑰模式下，內容順序: [32 bytes 原始檔案雜湊][12 bytes nonce][加密數據]
        header_len = HASH_LENGTH + 12 # 雜湊 + nonce
        if len(full_content) < header_len:
            return {"status": "error", "message": "invalid_encrypted_file_format", 
                            "details": "檔案格式不符隨機金鑰加密模式，可能是密碼加密檔或已損壞。"}

        nonce = full_content[HASH_LENGTH : HASH_LENGTH + 12]
        encrypted_data = full_content[header_len:]
        ...
        try:
            with open(key_to_load_path, 'rb') as f:
                key = f.read()
            
            aesgcm = AESGCM(key)
            decrypted_data = aesgcm.decrypt(nonce, encrypted_data, None)
            log_method = 'decrypt_with_keyfile'
            key_identifier_for_log = extracted_hash.hex() # 隨機金鑰解密的金鑰識別碼 (雜湊值)
        except FileNotFoundError:
